...
# Initialize AuthMethod var, see comment below in elif "auth" block
authmethodset = False
for inputtag in formsoup.find_all(re.compile('(INPUT|input)')):
    ...
    if "user" in name.lower():
        ...
        #Make an educated guess that this is the right field for the auth method
        # There are 2 AuthMethod tags in the response, so set the first and skip the next otherwise it gets overwritten to null
        if not authmethodset:
            payload[name] = value
            authmethodset = True
    else:
...
if 'SAMLResponse' not in response.text:
    ...
    otp = getpass.getpass(prompt='Enter your RSA SecurID passcode: ')
    formsoup = BeautifulSoup(response.text.decode('utf8'), "html.parser")
    payload = {}
    for inputtag in formsoup.find_all(re.compile('(INPUT|input)')):
        name = inputtag.get('name','')
        value = inputtag.get('value','')
        if "passcode" in name.lower():
            # OTP
            payload[name] = otp
        elif "auth" in name.lower():
            #Make an educated guess that this is the right field for the auth method
...
if (assertion == ''):
    ...
    # Write SAML response to a file for debugging
    #file = open("samlresponse.out","w")
...
# Debug only
#file = open("samlassertion.out","w")
#file.write(base64.b64decode(assertion))
#file.close()
